...
public class Phase3ResourcesTest {
  ...
  public void testPlaceIsAContainerWithCoordinates() {
    ...
    String loc = new JsonObject(authed().body(new JsonObject().put("name", "Garage").put("type", "location").encode())
        .post("/api/v1/items").then().statusCode(201).body("name", equalTo("Garage")).extract().asString())
        .getString("id");
    JsonObject pinned = new JsonObject(authed().get("/api/v1/items/" + loc).then().extract().asString())
        .put("latitude", 33.7).put("longitude", -84.4);
    authed().body(pinned.encode()).put("/api/v1/items/" + loc).then().statusCode(200).body("latitude", is(33.7f));
    ...
    authed().get("/api/v1/items/" + item + "/coordinates").then().statusCode(200).body("latitude", is(33.7f))
        .body("longitude", is(-84.4f));
  ...
  public void testParValuesRideItemJson() {
    ...
    authed().get("/api/v1/items/" + item).then().statusCode(200).body("parValues.minOnHand", is(5))
        .body("parValues.maxOnHand", is(50));
  }
  ...
  public void testAssetUploadDownloadDelete() {
    ...
    String assetId = new JsonObject(
        given().header("Authorization", "Bearer " + TOKEN).header(AssetsResource.FILENAME_HEADER, "pic.png")
            .contentType("image/png").body(photo).post("/api/v1/items/" + item + "/assets").then().statusCode(201)
            .body("filename", equalTo("pic.png")).body("sizeBytes", is(4)).extract().asString())
        .getString("id");

    authed().get("/api/v1/items/" + item + "/assets").then().statusCode(200).body("size()", is(1));

    byte[] roundTripped = given().header("Authorization", "Bearer " + TOKEN).get("/api/v1/assets/" + assetId).then()
        .statusCode(200).contentType("image/png").header(AssetsResource.FILENAME_HEADER, "pic.png").extract()
        .asByteArray();
    org.junit.jupiter.api.Assertions.assertArrayEquals(photo, roundTripped);

    // audit trail carries the attachment against the item
    authed().get("/api/v1/audit/target/" + item).then().statusCode(200).body("action",
        org.hamcrest.Matchers.hasItem("asset.attach"));

    authed().delete("/api/v1/assets/" + assetId).then().statusCode(204);
    authed().delete("/api/v1/assets/" + assetId).then().statusCode(404);
    given().header("Authorization", "Bearer " + TOKEN).header(AssetsResource.FILENAME_HEADER, "f")
        .contentType("image/png").body(photo).post("/api/v1/items/no-such-item/assets").then().statusCode(404);
  }
}
